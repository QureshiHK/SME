...
trackData = trackData.drop(trackData[(trackData.LEN < 10)].index) #remove all cycles below 10 frames length
trackData = trackData.drop(trackData[(trackData.LEN > 150)].index) #remove overlong cycles (for purpose we are interested in)
